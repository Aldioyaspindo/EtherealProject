            ...
            {catalogs.length === 0 ? (
              ...
              catalogs.map((catalog, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm">{catalog.productPrice}</td>
                  <td className="px-4 py-3 text-sm">{catalog.productColor}</td>
                  <td className="px-4 py-3 text-sm">{catalog.productSize}</td>
                  <td className="px-4 py-3 text-sm">{catalog.productTotal}</td>
                  <td className="px-4 py-3 text-sm">
                    {truncateText(catalog.productDescription, 50)}
                  </td>
                  <td className="px-4 py-3">
                    <Image
                      src={`${process.env.NEXT_PUBLIC_API_URL}/${catalog.productImage}`}
            ...
            >
              ...
              <div className="space-y-2 text-black">
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-lg">{catalog.productName}</h3>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p>{catalog.productPrice}</p>
                  </div>
                  <div>
                    <span className="font-semibold">Jumlah:</span>
                    <p>{catalog.productTotal}</p>
                  </div>
                  <div>
                    <span className="font-semibold">Warna:</span>
                    <p>{catalog.productColor}</p>
                  </div>
                  <div>
                    <span className="font-semibold">Ukuran:</span>
                    <p>{catalog.productSize}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-700">
                    {truncateText(catalog.productDescription, 80)}
                  </p>
                </div>
              </div>
